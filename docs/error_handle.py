...
'''
    当然，实际输入时可能会输入两个长度并不相等的数组。
    为了保证函数的鲁棒性，我们需要对异常情况进行处理。
    下面我们编写了两个函数；
    ...
    两种方法比较：
    方法1能够快速响应错误，但是需要额外一次check。
    方法2不需要额外check，但是需要等待真正报错时才会返回错误。
    > 当然，方法2只有当len(arr1)>len(arr2)的情况才会抛出异常
    > 存在缺陷，但是暂时不考虑这个问题

    疑问：
    如果采取方法2，如果能一开始就能检测出异常，何必要等到真正出错的时候才抛出异常。
    如果采取方法1，需要额外进行一次条件验证，影响性能？
'''
...
def main():
    ...
    # 使用compare1
    res = compare1(arr1, arr2)
    print("Result: ", res)


    # 使用compare2
    res = compare2(arr1, arr2)
    print("Result: ", res)
